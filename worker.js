    ...
    try {
      if (url.pathname === "/api/get-folders") {
        ...
        const { results } = await env.DB.prepare(
          "SELECT f.*, (SELECT nickname FROM accounts WHERE folder_id = f.id AND user_id = f.user_id AND platform = 'youtube' LIMIT 1) as youtube_channel FROM folders f WHERE f.user_id = ? ORDER BY f.created_at DESC"
        ).bind(userId).all();
