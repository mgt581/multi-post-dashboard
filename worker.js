...
export default {
  async fetch(request, env) {
    ...
    const trimmedBase = env.BASE_URL?.trim();
    // Prefer configured BASE_URL (if non-empty); otherwise fallback to current host
    const apiBase = (trimmedBase || baseUrl).replace(/\/$/, "");
